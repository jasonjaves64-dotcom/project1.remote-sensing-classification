...
这个项目是我在学习遥感与深度学习过程中的实践尝试。它尝试融合 Sentinel-2 光学时序、Sentinel-1 SAR 和 DEM 地形数据，训练一个像素级作物分类模型，并探索证据深度学习（EDL）在不确定性估计中的应用。
...
目前使用的是合成数据进行管线验证，真实数据上的训练和评估还在准备中。
...
| **数据** | 目前管线使用合成数据验证，真实数据训练待后续补充 |

> 📝 模型架构参考了多篇遥感与 CV 领域论文的思路，在此基础上做了自己的组合和尝试。具体参考见文末[致谢](#致谢)。
...
| V4 | 增加DEM | 三模态 + 双路时序 + Dropout 不确定性 |
| V5 | 代码整理 | 组件重构，修复早期设计中的几个问题 |
| V5EDL | 不确定性 | 引入 Dirichlet 证据学习，输出 vacuity/dissonance |
| V5Pro | 综合改进 | 可插拔骨干 + CARAFE 上采样 + 多尺度融合 |
| V6 | 继续尝试 | 层次化多尺度 + 多任务学习 + 时序编码优化 |

> V6 的许多组件还不够成熟，部分设计仍在实验中。具体设计文档见 `docs/` 目录。
...
推理时使用了 MC Dropout + TTA 来降低单次推理的随机性。这些都是从论文中学到的方法，在自己的数据上做了尝试。

---

## 消融实验论证

为了验证设计选择的合理性，在合成数据上进行了系统的消融实验（6 类实验、50+ 配置、16 组件消融）。完整报告见 [`docs/V6_EXPERIMENTS_REPORT.md`](docs/V6_EXPERIMENTS_REPORT.md)。
...
> 三模态融合优于任意子集，DEM 通过与光学/SAR 的交互产生价值。

![模态消融](ablation_output/figures/fig1a_modality_ablation.png)
...
| − Cross-Modal Attn | 0.0895 | +0.8% |

> **Early Fusion 是核心**：移除导致最大降幅。Cross-Modal Attention 在合成数据上作用有限。

![融合消融](ablation_output/figures/fig2_fusion_ablation.png)

### V6 组件增量贡献

从 V5EDL 基线开始逐步添加 V6 组件，量化每个组件的边际增益：
...
> V6 整体较 V5EDL 提升 **15.5%**，Early Fusion 贡献最大。

![组件消融](ablation_output/figures/fig5_component_ablation.png)

### 执行摘要

![总览](ablation_output/figures/fig0_summary_dashboard.png)

> ⚠️ **重要**: 所有实验均使用合成数据。mIoU 绝对值较低（~0.09）是因为合成数据使用随机标签，不代表真实场景性能。**相对差异**（组件增益/损失百分比）是可靠的论证依据。真实遥感数据验证待 V6.2 补充。
...
浏览器打开 https://jjjj111qq111-fusioncropnet-v6.hf.space 即可试用（无需安装，使用合成随机数据展示管线流程）。
...
本项目是公开仓库。已采取以下措施：
- `.env` 加入 `.gitignore`，仅提交 `.env.example` 占位符
- 数据库密码使用环境变量
- SQL 查询参数化
- 无真实密钥/Token/生产凭证提交

如发现安全问题，请提交 Issue，谢谢。
